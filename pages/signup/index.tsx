...
const LeftArea = ({ onClick }: HeaderLeftAreaProps) => {
    return <HeaderAreaContainer onClick={onClick}>{BackIcon()}</HeaderAreaContainer>;
};
...
const SignUpPage = () => {
    ...
    const backStep = () => (step > 0 ? setStep((step: number) => step - 1) : router.back());

    const submitSingup = async ({ birthDay, gender, nickName }: SignUpFormType) => {
